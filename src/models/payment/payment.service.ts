...
const createPayment = async (userId: string, payload: ICreatePayment) => {
  ...
  const { payment, booking, transactionId } = await prisma.$transaction(async (tx) => {
    ...
    if (isBookingExists.customerId !== userId) {
      throw new AppError(httpStatus.FORBIDDEN, "You are not authorized to pay for this booking");
    }
    ...
    if (isBookingExists.payment?.status === "COMPLETED") {
      throw new AppError(httpStatus.CONFLICT, "This booking has already been paid for");
    }
    ...
    return { payment: paymentRecord, booking: isBookingExists, transactionId: newTransactionId };
  });
  ...
  if (!session.url) {
    throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, "Failed to generate payment link");
  }
